...
coefficients, with no eigensolver in the optimization path, so the *entire
solve* is differentiable end-to-end (the property that lets a DFT calculation
sit inside a learning pipeline). The optimizer is any
