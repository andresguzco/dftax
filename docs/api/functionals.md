...
Functionals are Equinox modules selected by instance (`PBE()`) or by name
(`functional("wb97m-v")`, case- and punctuation-insensitive), spanning LDA,
GGA, meta-GGA, global-hybrid, and range-separated-hybrid rungs. The
...
ωB97M-V) gets the VV10 nonlocal-correlation double-grid term added on the
materialized XC grid.

::: dftax.energy.xc.functional
