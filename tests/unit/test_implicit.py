...
class TestImplicit:
    ...
    def test_implicit_forward_matches_scf(self):
        ...
        ks = RKS.from_molecule(mol, PBE(), gc, gw)
        P_impl = implicit_density(ks)
        P_ref = rks_scf(ks, e_tol=1e-10, d_tol=1e-9).P
    ...
    def test_implicit_gradient_equals_forces(self):
        ...
        def energy(coords):
            ...
            ks = RKS._assemble(basis, coords, charges, mol.nelectron, PBE(), g, w)
            return ks.total(implicit_density(ks))

        F_impl = -jax.grad(energy)(coords0)
        gc, gw = becke_grid(mol.symbols, mol.atom_coords(), NR, LEB)
        ks0 = RKS.from_molecule(mol, PBE(), gc, gw)
        C = rks_scf(ks0, e_tol=1e-10, d_tol=1e-9).mo_coeff[:, :nocc]
        F_ana = rks_forces(mol, PBE(), C, n_radial=NR, lebedev=LEB)
        assert float(jnp.max(jnp.abs(F_impl - F_ana))) < 1e-6

    def test_analytic_polarizability_matches_fd(self):
        mol = Molecule.from_xyz(WATER, "sto-3g")
        kw = dict(n_radial=60, lebedev=194, e_tol=1e-11, d_tol=1e-9)
